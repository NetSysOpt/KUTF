...
def process(m,files,epoch,tar_dir,pareto,device,optimizer,choose_weight=False,autoregression_iteration=1,training=True):
    if not training:
        return valid(m,files,epoch,tar_dir,pareto,device,optimizer,autoregression_iteration)
    else:
        return train(m,files,epoch,tar_dir,pareto,device,optimizer,choose_weight,autoregression_iteration)
    ...
    with torch.no_grad():
        with alive_bar(len(valid_files),title=f"Validating epoch {epoch}") as bar:
            for fnm in valid_files:
                ...
                bar()
    return avg_valid_loss, avg_sc




def train(m,train_files,epoch,train_tar_dir,pareto,device,optimizer,choose_weight,autoregression_iteration):
