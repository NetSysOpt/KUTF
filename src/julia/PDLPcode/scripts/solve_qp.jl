  ...
  function inner_solve()
    ...
    running_time = @elapsed begin
      output::FirstOrderLp.SaddlePointOutput =
        FirstOrderLp.optimize(parameters, lp, zeros(1), zeros(1), writemodel, filename, primal_weight, false)
    end
...
function parse_command_line()
  ...
  ArgParse.@add_arg_table! arg_parse begin
    ...
    "--writemodel"
    help = "whether or not to write the model"
    arg_type = Bool
    default = false
