...
function optimize(
  ...
  original_problem::QuadraticProgrammingProblem,
  primal_s_ws::Vector{Float64},
  dual_s_ws::Vector{Float64},
  writemodel::Bool,
  filename::String,         
  primal_weight=-1.0, 
  ...
  if writemodel
    open("../../../transformed_ins/train/$(filename)", "w") do file
        ori_mm = length(original_problem.right_hand_side)
        ori_nn = length(original_problem.objective_vector)

        mm = length(scaled_problem.scaled_qp.right_hand_side)
        nn = length(scaled_problem.scaled_qp.objective_vector)
    ...
    open("../../../ori_ins/train/$(filename)", "w") do file
        ...
        mm = length(original_problem.right_hand_side)
        nn = length(original_problem.objective_vector)
  ...
  # primal_s_ws = zeros(primal_size)
  # dual_s_ws = zeros(dual_size)
  if ~warmstart
    primal_s_ws = zeros(primal_size)
    dual_s_ws = zeros(dual_size)
  end
  current_dp = problem.constraint_matrix' * dual_s_ws

  # TODO: Correctly account for the number of kkt passes in
  # initialization
  solver_state = PdhgSolverState(
    primal_s_ws,  # current_primal_solution
    dual_s_ws,    # current_dual_solution
    zeros(primal_size),  # delta_primal
    zeros(dual_size),    # delta_dual
    # primal_s_ws,  # delta_primal
    # dual_s_ws,    # delta_dual
    current_dp,  # current_dual_product
